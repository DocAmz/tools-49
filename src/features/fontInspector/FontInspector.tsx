...
import { useFontUpload } from "./hooks/use-font";
...
import { ErrorMessage } from "@/types/fonts";
...
export default function FontInspector() {
  ...
  const handleClear = () => {
    clearFont();
    console.log("Clear font");
  };
  /**
  const handleDownload = () => {
    const file = getFontFile();
    if (file) {
  ...
  const getErrorMessageType = (error: string) => {
  ...
  const getErrorMessageTitle = (type: string) => {
    switch (type) {
      case "timeout":
        return "Font took too long to load";
      case "network":
        return "Network error loading font";
      case "format":
        return "Invalid font format";
      default:
  ...
  useEffect(() => {
    if (error) {
      const type = getErrorMessageType(error.message);
      const title = getErrorMessageTitle(type);
      setErrorMessage({
        title: title,
    ...
    return (
      <div className="container mx-auto p-4 space-y-4">
        <FontToolbar
          ...
          loading={loading}
        />
        <DropZone onFileSelect={noop} />
      </div>
    );
  }

  return (
    <div className="container h-[80vh] mx-auto flex flex-col ">
      <FontToolbar
        onClear={handleClear}
        onDownload={noop}
        onEdit={handleEdit}
      ...
      <div className="flex items-center justify-center p-4 mt-2">
        <Alert variant="default" className="space-x-4" >
          <NotepadTextIcon className="w-6 h-6 mr-2" />
          <AlertTitle>{'Coucou'}</AlertTitle>
          <AlertDescription>{'je suis une alert'}</AlertDescription>
        </Alert>
      </div>
      <div className="flex-1 overflow-hidden h-full">
        <div className="grid h-full grid-cols-1 gap-4 p-4 md:grid-cols-[350px_1fr]">
          <Card className="h-full overflow-auto rounded-none">
            {/*
            <FontInfo
              fontData={fontData}
              error={errorMessage}
          ...
          <Card className="h-full overflow-auto rounded-none">
            {loading ? (
              <div className="flex items-center justify-center h-full">
                <svg
                  className="animate-spin border-primary"
              ...
              <>
              {
                errorMessage ? (
                  <div className="h-full w-full flex items-center justify-center">
                    <Image src={'/text-error.svg'} width={300} height={300} alt="font error" />
                  </div>
                ) : (
                  <Tabs defaultValue="plain" className="h-full">
                ...
                <div className="p-4">
                  <TabsContent value="plain" className="m-0">
                    <PlainView
                      fontFamily={fontInfo?.family || "inherit"}
                      isEditing={isEditing}
                    />
                  </TabsContent>
                  <TabsContent value="waterfall" className="m-0">
                    <WaterfallView
                      fontFamily={fontInfo?.family || "inherit"}
                      isEditing={isEditing}
                    />
                  </TabsContent>
                  <TabsContent value="glyphs" className="m-0">
                    {/*
                    <GlyphGrid
                      glyphs={fontGlyphs || []}
                      fontFamily={fontInfo?.family || "inherit"}
