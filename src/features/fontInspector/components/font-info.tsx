...
export function FontInfo({ fontData, error, loading }: FontInfoProps) {
  if (!fontData && !error) return null;

  const sections: Section[] = [
    {
      title: "Basic Information",
      data: {
        "Font Family": fontData?.names?.fontFamily?.en,
        "Full Name": fontData?.names?.fullName?.en,
        Version: fontData?.names?.version?.en,
        Format: fontData?.outlinesFormat,
    ...
    },
    {
      ...
      data: {
        ...
        "Number of Glyphs": fontData?.numGlyphs,
      },
    },
    {
      title: "Author",
      data: {
        Designer: fontData?.names?.designer?.en,
        Manufacturer: fontData?.names?.manufacturer?.en,
        Copyright: fontData?.names?.copyright?.en,
      },
    },
  ];
  ...
  const ErrorDisplay = () =>
    error && (
      <div className="h-full flex items-center justify-center p-4 mb-6 border-2 border-red-500 bg-background">
        <div className="flex-col items-center justify-center">
          <div className="w-full flex items-center space-x-4">
            <div className="flex-shrink-0 bg-red-50 border border-red-500 rounded-full p-3">
              <svg
            ...
            <div className="flex-1">
              <h3 className=" font-bold text-red-500">{error.title}</h3>
              <div className="mt-1">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex-1">
              <p className="mt-2 text-sm text-5ed-700">{error.message}</p>
              <Table>
                  <TableBody>
                  {error.details && (
                    <TableRow className="mt-2 space-y-1">
                      {error.details.map((detail, index) => (
                        <TableCell key={index} className="text-sm text-red-700">
                          {detail}
                        </TableCell>
                      ))}
                    </TableRow>
              ...
              <Table>
                <TableBody>
                  {Object.entries(section.data).map(([key, value], i) => {
                    if(isEven(i)) {
                      return (
                        <TableRow key={key} className="bg-background">
